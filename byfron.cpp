...
function getthreadcontext() return nyx_context end

function setthreadidentity(identity) return nyx_context end
function setidentity(identity) return nyx_context end
function setthreadcontext(identity) return nyx_context end
...
function isgameactive() return nyx_active end

function info(printtt) print(printtt) end

function islclosure(func) assert(type(func) == "function", "invalid argument #1 to 'islclosure' (function expected, got " .. type(func) .. ") ", 2) local success = pcall(function() return setfenv(func, getfenv(func)) end) return success end
function iscclosure(func) assert(type(func) == "function", "invalid argument #1 to 'iscclosure' (function expected, got " .. type(func) .. ") ", 2) return not islclosure(func) end
function newlclosure(f) assert(type(f) == "function", "invalid argument #1 to 'newlclosure' (function expected, got " .. type(f) .. ") ", 2) return function(...) return f(...) end end

function fireproximityprompt(Obj, Amount, Skip) assert(typeof(Obj) == "Instance", "invalid argument #1 to 'fireproximityprompt' (ProximityPrompt expected, got " .. type(Spoof) .. ") ") assert(Obj.ClassName == "ProximityPrompt", "invalid argument #1 to 'fireproximityprompt' (ProximityPrompt expected, got " .. type(Spoof) .. ") ") assert(type(Amount) == "number", "invalid argument #2 to 'fireproximityprompt' (number expected, got " .. type(Amount) .. ") ", 2) Amount = Amount or 1 local PromptTime = Obj.HoldDuration if Skip then Obj.HoldDuration = 0 end for i = 1, Amount do Obj:InputHoldBegin() if not Skip then wait(Obj.HoldDuration) end Obj:InputHoldEnd() end Obj.HoldDuration = PromptTime end
function fireclickdetector(part) local cd = part:FindFirstChild("ClickDetector") or part local oldParent = cd.Parent local p = Instance.new("Part") p.Transparency = 1 p.Size = Vector3.new(30,30,30) p.Anchored = true p.CanCollide = false p.Parent = workspace cd.Parent = p cd.MaxActivationDistance = math.huge local conn conn = game["Run Service"].Heartbeat:Connect(function() p.CFrame = workspace.Camera.CFrame *CFrame.new(0,0,-20) * CFrame.new(workspace.Camera.CFrame.LookVector.X,workspace.Camera.CFrame.LookVector.Y,workspace.Camera.CFrame.LookVector.Z) game:GetService("VirtualUser"):ClickButton1(Vector2.new(20,20), workspace:FindFirstChildOfClass("Camera").CFrame) end) cd.MouseClick:Once(function() conn:Disconnect() cd.Parent = oldParent p:Destroy() end) end
...
function getrunningscripts() local returnable = {} for _,v in game:GetDescendants() do if v:IsA("LocalScript") or v:IsA("ModuleScript") then if not v:IsA("ModuleScript") and v.Enabled then returnable[#returnable + 1] = v end if v:IsA("ModuleScript") then returnable[#returnable + 1] = v end end end return returnable end
...
mousemoverel = function(relx, rely)
    local vim = game:GetService('VirtualInputManager');
...
type Streamer = { Offset: number, Source: string, Length: number, IsFinished: boolean, LastUnreadBytes: number, read: (Streamer, len: number?, shiftOffset: boolean?) -> string, seek: (Streamer, len: number) -> (), append: (Streamer, newData: string) -> (), toEnd: (Streamer) -> () } type BlockData = { [number]: { Literal: string, LiteralLength: number, MatchOffset: number?, MatchLength: number? } }
local function plainFind(str, pat) return string.find(str, pat, 0, true) end
local function streamer(str): Streamer local Stream = {} Stream.Offset = 0 Stream.Source = str Stream.Length = string.len(str) Stream.IsFinished = false Stream.LastUnreadBytes = 0 function Stream.read(self: Streamer, len: number?, shift: boolean?): string local len = len or 1 local shift = if shift ~= nil then shift else true local dat = string.sub(self.Source, self.Offset + 1, self.Offset + len) local dataLength = string.len(dat) local unreadBytes = len - dataLength if shift then self:seek(len) end self.LastUnreadBytes = unreadBytes return dat end function Stream.seek(self: Streamer, len: number) local len = len or 1 self.Offset = math.clamp(self.Offset + len, 0, self.Length) self.IsFinished = self.Offset >= self.Length end function Stream.append(self: Streamer, newData: string) self.Source ..= newData self.Length = string.len(self.Source) self:seek(0) end function Stream.toEnd(self: Streamer) self:seek(self.Length) end return Stream end
function lz4compress(str: string): string local blocks: BlockData = {} local iostream = streamer(str) if iostream.Length > 12 then local firstFour = iostream:read(4) local processed = firstFour local lit = firstFour local match = "" local LiteralPushValue = "" local pushToLiteral = true repeat pushToLiteral = true local nextByte = iostream:read() if plainFind(processed, nextByte) then local next3 = iostream:read(3, false) if string.len(next3) < 3 then LiteralPushValue = nextByte .. next3 iostream:seek(3) else match = nextByte .. next3 local matchPos = plainFind(processed, match) if matchPos then iostream:seek(3) repeat local nextMatchByte = iostream:read(1, false) local newResult = match .. nextMatchByte local repos = plainFind(processed, newResult) if repos then match = newResult matchPos = repos iostream:seek(1) end until not plainFind(processed, newResult) or iostream.IsFinished local matchLen = string.len(match) local pushMatch = true if iostream.Length - iostream.Offset <= 5 then LiteralPushValue = match pushMatch = false end if pushMatch then pushToLiteral = false local realPosition = string.len(processed) - matchPos processed = processed .. match table.insert(blocks, { Literal = lit, LiteralLength = string.len(lit), MatchOffset = realPosition + 1, MatchLength = matchLen, }) lit = "" end else LiteralPushValue = nextByte end end else LiteralPushValue = nextByte end if pushToLiteral then lit = lit .. LiteralPushValue processed = processed .. nextByte end until iostream.IsFinished table.insert(blocks, { Literal = lit, LiteralLength = string.len(lit) }) else local str = iostream.Source blocks[1] = { Literal = str, LiteralLength = string.len(str) } end local output = string.rep("\x00", 4) local function write(char) output = output .. char end for chunkNum, chunk in blocks do local litLen = chunk.LiteralLength local matLen = (chunk.MatchLength or 4) - 4 local tokenLit = math.clamp(litLen, 0, 15) local tokenMat = math.clamp(matLen, 0, 15) local token = bit32.lshift(tokenLit, 4) + tokenMat write(string.pack("<I1", token)) if litLen >= 15 then litLen = litLen - 15 repeat local nextToken = math.clamp(litLen, 0, 0xFF) write(string.pack("<I1", nextToken)) if nextToken == 0xFF then litLen = litLen - 255 end until nextToken < 0xFF end write(chunk.Literal) if chunkNum ~= #blocks then write(string.pack("<I2", chunk.MatchOffset)) if matLen >= 15 then matLen = matLen - 15 repeat local nextToken = math.clamp(matLen, 0, 0xFF) write(string.pack("<I1", nextToken)) if nextToken == 0xFF then matLen = matLen - 255 end until nextToken < 0xFF end end end local compLen = string.len(output) - 4 local decompLen = iostream.Length return string.pack("<I4", compLen) .. string.pack("<I4", decompLen) .. output end
function lz4decompress(lz4data: string): string local inputStream = streamer(lz4data) local compressedLen = string.unpack("<I4", inputStream:read(4)) local decompressedLen = string.unpack("<I4", inputStream:read(4)) local reserved = string.unpack("<I4", inputStream:read(4)) if compressedLen == 0 then return inputStream:read(decompressedLen) end local outputStream = streamer("") repeat local token = string.byte(inputStream:read()) local litLen = bit32.rshift(token, 4) local matLen = bit32.band(token, 15) + 4 if litLen >= 15 then repeat local nextByte = string.byte(inputStream:read()) litLen += nextByte until nextByte ~= 0xFF end local literal = inputStream:read(litLen) outputStream:append(literal) outputStream:toEnd() if outputStream.Length < decompressedLen then local offset = string.unpack("<I2", inputStream:read(2)) if matLen >= 19 then repeat local nextByte = string.byte(inputStream:read()) matLen += nextByte until nextByte ~= 0xFF end outputStream:seek(-offset) local pos = outputStream.Offset local match = outputStream:read(matLen) local unreadBytes = outputStream.LastUnreadBytes local extra if unreadBytes then repeat outputStream.Offset = pos extra = outputStream:read(unreadBytes) unreadBytes = outputStream.LastUnreadBytes match ..= extra until unreadBytes <= 0 end outputStream:append(match) outputStream:toEnd() end until outputStream.Length >= decompressedLen return outputStream.Source end
...
function queueonteleport() end

function setclipboard(text)
    local response = HttpGet("http://localhost:6969/setclipboard?path=" .. text)
    return response
end

function messagebox(text) print(text) warn(text) end

local RunService = game:GetService("RunService")
local frameTime = 1 / 60 
local capped = false

function setfpscap(fps) if fps == 0 then capped = false RunService:Set3dRenderingEnabled(true) else frameTime = 1 / fps capped = true RunService:Set3dRenderingEnabled(false) end end
function getfpscap() return 60 end
local function onRenderStepped() if capped then local targetTime = tick() + frameTime while tick() < targetTime do wait() end end end

function getgc() local garbageCollection = { {name = "NyxGC", type = "Part"}, {name = "NyxGC2", type = "Script"}, } return garbageCollection end
function getreg() return reg end
...
function clonefunction(p1) assert(type(p1) == "function", "invalid argument #1 to 'clonefunction' (function expected, got " .. type(p1) .. ") ", 2) local A = p1 local B = xpcall(setfenv, function(p2, p3) return p2, p3 end, p1, getfenv(p1)) if B then return function(...) return A(...) end end return coroutine.wrap(function(...) while true do A = coroutine.yield(A(...)) end end) end
...
function getrenv() return renv end
...
crypt.base64_decode = function(data)
    local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
    data = string.gsub(data, '[^'..b..'=]', '')
    return (data:gsub('.', function(x)
        if x == '=' then return '' end
...
encrypt64k = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
crypt.base64encode = function(data)
    return ((data:gsub('.', function(x) 
        ...
        return encrypt64k:sub(c+1,c+1)
    end)..({ '', '==', '=' })[#data%3+1])
end
crypt.base64_encode = function(data)
    return ((data:gsub('.', function(x) 
        local r,b='',x:byte()
        for i=8,1,-1 do r=r..(b%2^i-b%2^(i-1)>0 and '1' or '0') end
        return r;
...
crypt.generatebytes = function(size: number): string
...
crypt.base64.decode = function(data)
    local b = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
    data = string.gsub(data, '[^'..b..'=]', '')
    return (data:gsub('.', function(x)
        if x == '=' then return '' end
...
encrypt64k = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
crypt.base64.encode = function(data)
    return ((data:gsub('.', function(x) 
        local r,b='',x:byte()
        for i=8,1,-1 do r=r..(b%2^i-b%2^(i-1)>0 and '1' or '0') end
        return r;
...
local Response Start(Request, function(state, response) Response = response Event:Fire() end) Event.Event:Wait() return Response end
...
function isexecutorclosure(func)
    ...
    return f_env.script.Parent == nil or f_env == getfenv(0) 
end
...
cache.invalidate = function(instance)
    if typeof(instance) == "Instance" then
        ...
        error("Provided argument is not an instance.")
    end
    return false
end
...
function writefile(path, content)
    local response = HttpGet("http://localhost:6969/writefile?path=" .. path .. "&text=" .. content)
    --return response
end

function makefolder(path)
    local response = HttpGet("http://localhost:6969/makefolder?path=" .. path)
    return response
end

function isfolder(path)
    local response = HttpGet("http://localhost:6969/isfolder?path=" .. path)
    local bleh = true
...
function isfile(path)
    local response = HttpGet("http://localhost:6969/isfile?path=" .. path)
    local bleh = true
    if response == "true" then
        bleh = true
    end
...
function readfile(path)
    local content = HttpGet("http://localhost:6969/readfile?path=" .. path)
    return content
end

function appendfile(path, text2)
...
function delfolder(path)
    local response = HttpGet("http://localhost:6969/delfolder?path=" .. path)
   -- return response
end
function delfile(path)
    local response = HttpGet("http://localhost:6969/delfile?path=" .. path)
    --return response
end
function listfiles(path)
    local response = HttpGet("http://localhost:6969/listfiles?path=" .. path)
    local files_table = game:GetService("HttpService"):JSONDecode(response)
    return files_table
end

function system(cmd)
    local response = HttpGet("http://localhost:6969/system?path=" .. path)
    --return response
end														

--[[
local TweenService = game:GetService("TweenService")
local function NYXGUI()
	...
	local NYXTEXT = Instance.new("TextLabel")
	...
	NYXLOGO.Parent = NYX
	NYXLOGO.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	NYXLOGO.BackgroundTransparency = 1.000
	NYXLOGO.BorderColor3 = Color3.fromRGB(0, 0, 0)
	NYXLOGO.BorderSizePixel = 0
	NYXLOGO.Position = UDim2.new(0.463735342, 0, 0.428657413, 0)
	NYXLOGO.Size = UDim2.new(0, 138, 0, 138)
	NYXLOGO.Image = "rbxassetid://18166194583"

	NYXTEXT.Name = "NYXTEXT"
	NYXTEXT.Parent = NYXLOGO
	NYXTEXT.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	NYXTEXT.BackgroundTransparency = 1.000
	NYXTEXT.BorderColor3 = Color3.fromRGB(0, 0, 0)
	NYXTEXT.BorderSizePixel = 0
	NYXTEXT.Position = UDim2.new(-0.304347813, 0, 0.934782624, 0)
	NYXTEXT.Size = UDim2.new(0, 222, 0, 33)
	NYXTEXT.Font = Enum.Font.Unknown
	NYXTEXT.Text = ""
	NYXTEXT.TextColor3 = Color3.fromRGB(255, 255, 255)
	NYXTEXT.TextScaled = true
	NYXTEXT.TextSize = 14.000
	NYXTEXT.TextStrokeTransparency = 0.700
	NYXTEXT.TextWrapped = true

	wait(2)
	local tweenInfo = TweenInfo.new(2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0)

	local transparencyTween = TweenService:Create(NYXLOGO, tweenInfo, {ImageTransparency = 2})
	local transparencyTween2 = TweenService:Create(blur, tweenInfo, {Size = -25})

	transparencyTween:Play()
	transparencyTween2:Play()
	transparencyTween.Completed:Connect(function()
	if not transparencyTween2.Completed then return end
		--NYX:Destroy()
	end)
...
local loadstring = newcclosure(function(source)
	...
	local ran, failureReason = pcall(function()
		local compiledBytecode = compile(source, "Nyx")
